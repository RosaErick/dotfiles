...
# Plugins installed by scripts/install-plugins.sh; distro paths are fallbacks.
ZSH_AUTOSUGGEST_STRATEGY=(history completion)
ZSH_AUTOSUGGEST_HIGHLIGHT_STYLE='fg=#6f6f6f'
ZSH_AUTOSUGGEST_BUFFER_MAX_SIZE=20

_dotfiles_zsh_plugin() {
  local plugin="$1" base
  for base in "${XDG_DATA_HOME:-$HOME/.local/share}/zsh/plugins" \
      /usr/share/zsh/plugins /usr/share /opt/homebrew/share /usr/local/share; do
    if [[ -r "$base/$plugin/$plugin.zsh" ]]; then
...
_dotfiles_zsh_plugin zsh-autosuggestions && bindkey '^ ' autosuggest-accept

[[ -r "$HOME/.config/zsh/local.zsh" ]] && source "$HOME/.config/zsh/local.zsh"
# Syntax highlighting must load last.
_dotfiles_zsh_plugin zsh-syntax-highlighting
unfunction _dotfiles_zsh_plugin
